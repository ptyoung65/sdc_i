...
if __name__ == "__main__":
    ...
    # 테스트 문서
    test_content = """
    머신러닝은 인공지능의 핵심 기술이다. 딥러닝은 머신러닝의 한 분야로, 인공신경망을 사용한다.
    구글은 TensorFlow라는 머신러닝 프레임워크를 개발했다. TensorFlow는 딥러닝 모델 구축에 널리 사용된다.
    삼성전자도 AI 기술 개발에 투자하고 있다. 삼성은 Bixby라는 AI 어시스턴트를 만들었다.
    """
    
    # 그래프 구축
    result = builder.build_graph_from_document(
    ...
    print(f"추출된 개체: {result['entities_extracted']}")
    print(f"추출된 관계: {result['relations_extracted']}")
    print(f"추가된 노드: {result['nodes_added']}")
    print(f"추가된 엣지: {result['edges_added']}")
    
    # 쿼리 테스트
    query_result = builder.query_graph("머신러닝과 딥러닝의 관계는?")
    print(f"\n=== 쿼리 결과 ===")
    print(f"관련 서브그래프 수: {len(query_result)}")
    
    for i, subgraph_info in enumerate(query_result[:2]):
        print(f"\n서브그래프 {i+1}:")
        print(f"중심 개체: {subgraph_info['central_entity']}")
        print(f"관련성 점수: {subgraph_info['relevance_score']:.2f}")
    
    # 컨텍스트 생성
    context = builder.generate_graph_context(query_result)
    print(f"\n=== 생성된 컨텍스트 ===")
    print(context)
    ...
    print(f"개체 유형: {stats['entity_types']}")
    print(f"관계 유형: {stats['relation_types']}")