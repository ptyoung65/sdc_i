...
import { DualProviderDisplay } from "./dual-provider-display"
...
export function ChatInterface({ conversation, className }: ChatInterfaceProps) {
  ...
  const [selectedProviders, setSelectedProviders] = React.useState<string[]>(['gemini'])
  ...
  const [useInternalServers, setUseInternalServers] = React.useState(false)
  const [internalServerMode, setInternalServerMode] = React.useState<'embedding' | 'llm' | 'hybrid'>('hybrid')
  ...
  const [multiProviderResponses, setMultiProviderResponses] = React.useState<{
    [key: string]: string
    isLoading: boolean
  }>({ isLoading: false })
  ...
  const handleSendMessage = async (content: string, files?: File[]) => {
    ...
    try {
      ...
      if (selectedProviders.length > 1) {
        setMultiProviderResponses({ isLoading: true })
        ...
        const baseRequest = {
          ...
          use_internal_servers: useInternalServers,
          internal_server_mode: internalServerMode,
        ...
        try {
          ...
          const providerCalls = selectedProviders.map(providerId =>
            apiService.sendMessage({ ...baseRequest, provider: providerId })
          )

          const responses = await Promise.allSettled(providerCalls)
          ...
          selectedProviders.forEach((providerId, index) => {
            ...
            results[providerId] = response.status === 'fulfilled' && response.value.success
              ? response.value.response
              : '응답을 가져올 수 없습니다.'
          })

          setMultiProviderResponses({
            ...results,
            isLoading: false
          })

          // Create a combined response message for the chat history
          const combinedResponse = selectedProviders.map(providerId =>
            `**${providerNames[providerId] || providerId} 결과:**\n${results[providerId]}`
          ).join('\n\n')
          ...
          const assistantMessage: Message = {
            ...
            timestamp: new Date().toISOString(),
            conversationId: currentConversationId || generateId(),
            metadata: {
              model: `multi-provider-${selectedProviders.join('-')}`,
              processingTime: 2.5,
              confidence: 0.9,
          ...
          console.error('Error in multi-provider call:', error)
          setMultiProviderResponses({ isLoading: false })
          throw error
        }
      }

      // Regular single provider call for non-web modes
      ...
      // 내부 서버 및 외부 LLM 오류 처리 개선
      const isInternalServerError = response.error_type &&
        ['connection_failed', 'timeout', 'auth_failed', 'endpoint_not_found', 'server_error', 'unknown_error'].includes(response.error_type)

      const isExternalLLMError = response.error_type === 'external_api_error'
      const isAnyServerError = isInternalServerError || isExternalLLMError
      ...
      const assistantMessage: Message = {
        ...
        metadata: {
          model: response.model || selectedProviders[0],
          processingTime: 1.2,
          confidence: 0.95,
          ...(response.tokens && {
            tokens: {
          ...
          // 내부 서버 정보 및 오류 상태 추가
          ...(response.server_used && {
            serverUsed: response.server_used,
            serverUrl: response.server_url,
            isInternalServer: true
          }),
          // 외부 LLM API 정보 추가
          ...(response.is_external_api && {
        ...
        ...(isInternalServerError && {
          error: `내부 서버 연결 문제: ${response.error_type}`
        }),
        ...(isExternalLLMError && {
          error: `외부 LLM API 오류: ${response.error_type}`
        })
      }
    ...
    <div className={`flex flex-col h-full ${className}`}>
      ...
      <div className="flex-1 overflow-hidden">
        <ScrollArea ref={scrollAreaRef} className="h-full">
          <div className="flex flex-col">
            ...
            {/* Dual Provider Display for Web Mode */}
            {searchMode === 'web' && (dualResponses.isLoading || dualResponses.gpt || dualResponses.perplexity) && (
              <motion.div
              ...
              >
                <DualProviderDisplay
                  gptResponse={dualResponses.gpt}
                  perplexityResponse={dualResponses.perplexity}
                  isLoading={dualResponses.isLoading}
                />
              </motion.div>
            )}

            {/* Typing Indicator */}
            {isLoading && searchMode !== 'web' && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
      ...
      <div className="shrink-0 border-t bg-background/80 backdrop-blur-sm p-4">
        ...
        <div className="flex flex-col gap-3 mb-3">
          {/* LLM Provider Selector - Multi-select */}
          {providers.length > 0 && (
                    ...
                    onClick={() => {
                      setSelectedProviders(prev => {
                        if (prev.includes(provider.id)) {
                          // 최소 하나는 선택되어야 함
                          return prev.length > 1 ? prev.filter(id => id !== provider.id) : prev
                        } else {
          ...
          {/* Internal Server Options */}
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground">내부서버:</span>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={useInternalServers}
                onChange={(e) => setUseInternalServers(e.target.checked)}
                className="w-4 h-4 rounded border-gray-300"
              />
              <span className="text-xs text-muted-foreground">
                내부 서버 {useInternalServers ? '활성' : '비활성'}
              </span>
            </label>
            {useInternalServers && (
              <div className="flex items-center gap-1 ml-2">
                ...
                <select
                  value={internalServerMode}
                  onChange={(e) => setInternalServerMode(e.target.value as 'embedding' | 'llm' | 'hybrid')}
                  className="px-2 py-1 text-xs rounded border bg-background"
                >
                  <option value="embedding">임베딩만</option>
                  <option value="llm">LLM만</option>
