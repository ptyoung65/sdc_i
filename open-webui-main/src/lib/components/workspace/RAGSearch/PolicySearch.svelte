<script lang="ts">
	import { searchByDataType, type SearchResult } from '$lib/apis/rag-search';
	import { onMount } from 'svelte';

	let query = '';
	let searchOption = 'all';
	let results: SearchResult[] = [];
	let loading = false;
	let error = '';
	let totalPolicies = 0;
	let totalChapters = 0;

	// 필터
	let policyTypes = {
		hr: true,
		finance: true,
		operation: true,
		security: true
	};

	async function handleSearch() {
		if (!query.trim()) return;

		loading = true;
		error = '';

		try {
			const response = await searchByDataType(query, 'policy', 2);
			results = response.results;
		} catch (err: any) {
			error = err.message || '검색 중 오류가 발생했습니다';
			results = [];
		} finally {
			loading = false;
		}
	}

	function handleKeyPress(e: KeyboardEvent) {
		if (e.key === 'Enter') {
			handleSearch();
		}
	}

	onMount(() => {
		totalPolicies = 89;
		totalChapters = 234;
	});
</script>

<div class="flex flex-col h-full">
	<!-- 헤더 -->
	<div class="mb-4">
		<h2 class="text-2xl font-semibold flex items-center gap-2">
			📋 회사사규 검색
		</h2>
		<p class="text-sm text-gray-500 mt-1">회사 규정과 절차를 검색하세요</p>
	</div>

	<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
		<!-- 검색 패널 -->
		<div class="flex flex-col gap-4">
			<!-- 통계 -->
			<div class="grid grid-cols-3 gap-3">
				<div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg text-center">
					<div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{totalPolicies}</div>
					<div class="text-xs text-gray-600 dark:text-gray-400 mt-1">규정 문서</div>
				</div>
				<div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg text-center">
					<div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{totalChapters}</div>
					<div class="text-xs text-gray-600 dark:text-gray-400 mt-1">장/조</div>
				</div>
				<div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg text-center">
					<div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{results.length}</div>
					<div class="text-xs text-gray-600 dark:text-gray-400 mt-1">검색 결과</div>
				</div>
			</div>

			<!-- 검색 폼 -->
			<div class="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
				<div class="space-y-4">
					<!-- 검색어 입력 -->
					<div>
						<label class="block text-sm font-medium mb-2">규정 또는 절차 검색</label>
						<textarea
							bind:value={query}
							on:keypress={handleKeyPress}
							placeholder="예: 휴가 신청, 출장 규정, 복무 규정..."
							rows="4"
							class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-blue-500"
						/>
					</div>

					<!-- 규정 유형 필터 -->
					<div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
						<h3 class="text-sm font-medium mb-3">규정 유형</h3>
						<div class="space-y-2">
							<label class="flex items-center gap-2 cursor-pointer">
								<input type="checkbox" bind:checked={policyTypes.hr} class="rounded" />
								<span class="text-sm">인사/복무</span>
							</label>
							<label class="flex items-center gap-2 cursor-pointer">
								<input type="checkbox" bind:checked={policyTypes.finance} class="rounded" />
								<span class="text-sm">재무/회계</span>
							</label>
							<label class="flex items-center gap-2 cursor-pointer">
								<input type="checkbox" bind:checked={policyTypes.operation} class="rounded" />
								<span class="text-sm">업무/운영</span>
							</label>
							<label class="flex items-center gap-2 cursor-pointer">
								<input type="checkbox" bind:checked={policyTypes.security} class="rounded" />
								<span class="text-sm">보안/정보</span>
							</label>
						</div>
					</div>

					<!-- 검색 옵션 -->
					<div>
						<label class="block text-sm font-medium mb-2">검색 옵션</label>
						<select
							bind:value={searchOption}
							class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800"
						>
							<option value="all">전체 검색</option>
							<option value="current">현행 규정만</option>
							<option value="recent">최근 6개월 내</option>
						</select>
					</div>

					<!-- 검색 버튼 -->
					<button
						on:click={handleSearch}
						disabled={loading || !query.trim()}
						class="w-full px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
					>
						{#if loading}
							<span class="flex items-center justify-center gap-2">
								<svg class="animate-spin h-5 w-5" viewBox="0 0 24 24">
									<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none" />
									<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
								</svg>
								<span>검색 중...</span>
							</span>
						{:else}
							🔍 규정 검색
						{/if}
					</button>
				</div>
			</div>
		</div>

		<!-- 검색 결과 -->
		<div class="bg-white dark:bg-gray-900 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
			<h3 class="text-lg font-semibold mb-4 flex items-center gap-2">
				검색 결과
				<span class="text-sm text-gray-500">({results.length}개)</span>
			</h3>

			{#if error}
				<div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded-lg">
					❌ {error}
				</div>
			{:else if results.length === 0}
				<div class="text-center py-12 text-gray-500">
					<div class="text-6xl mb-4">📋</div>
					<p>회사 규정을 검색하세요</p>
				</div>
			{:else}
				<div class="space-y-3 max-h-[600px] overflow-y-auto">
					{#each results as result, idx}
						<div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border-l-4 border-green-500">
							<div class="flex items-start justify-between mb-2">
								<div class="font-semibold text-lg flex items-center gap-2">
									<span>📋</span>
									<span>{result.metadata?.chapter || '규정'}</span>
								</div>
								<div class="bg-green-600 text-white text-xs px-2 py-1 rounded-full">
									{((result.final_score || result.score) * 100).toFixed(1)}%
								</div>
							</div>

							<div class="flex items-center gap-4 text-xs text-gray-600 dark:text-gray-400 mb-2">
								<span>🏢 {result.metadata?.department || '전사'}</span>
								<span>📅 {result.metadata?.effective_date || '현행'}</span>
								<span>📌 {result.metadata?.article || '조항'}</span>
							</div>

							<div class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed mb-3">
								{result.text}
							</div>

							<div class="flex flex-wrap gap-2">
								{#if result.metadata?.hierarchy}
									<span class="bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 text-xs px-2 py-1 rounded">
										🔗 {result.metadata.hierarchy}
									</span>
								{/if}
								{#if result.metadata?.version}
									<span class="bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 text-xs px-2 py-1 rounded">
										v{result.metadata.version}
									</span>
								{/if}
								{#if result.metadata?.status}
									<span class="bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300 text-xs px-2 py-1 rounded">
										{result.metadata.status}
									</span>
								{/if}
							</div>
						</div>
					{/each}
				</div>
			{/if}
		</div>
	</div>
</div>
