<script lang="ts">
	...
	import ExternalModelConfirmModal from '../ExternalModelConfirmModal.svelte';
	...
	let showExternalConfirmModal = false;

	let ollamaVersion = null;
	let selectedModelIdx = 0;

	// External 모델 전환 확인 핸들러
	const handleExternalClick = () => {
		showExternalConfirmModal = true;
	};

	// 사용자가 동의하면 실제로 External 모델로 전환
	const handleExternalConfirm = () => {
		selectedTag = '';
		selectedConnectionType = 'external';
		modelType.set('external');
	};
		...
		searchValue
			? fuse
					...
					.filter((item) => {
						...
						const isChatGptOrPerplexity = modelId.includes('gpt-') || modelId.includes('pplx-');

						if (selectedConnectionType === '') {
							// 전체 탭: ChatGPT/Perplexity 제외 (외부모델 탭에서만 표시)
							return !isChatGptOrPerplexity;
						} else if (selectedConnectionType === 'local') {
							// 내부모델 탭: local이면서 ChatGPT/Perplexity 아닌 것만
							return item.model?.connection_type === 'local' && !isChatGptOrPerplexity;
						} else if (selectedConnectionType === 'external') {
							// 외부모델 탭: external이면서 ChatGPT/Perplexity만
							return item.model?.connection_type === 'external' && isChatGptOrPerplexity;
...
<!-- 외부모델 전환 확인 팝업 -->
<ExternalModelConfirmModal
	bind:show={showExternalConfirmModal}
	on:confirm={handleExternalConfirm}
	on:cancel={() => {
		showExternalConfirmModal = false;
	}}
/>
