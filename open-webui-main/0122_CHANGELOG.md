...
토큰/턴 제한으로 세션이 초기화될 때, 현재 대화 내용을 AI 모델을 사용하여 약 4000 토큰 분량으로 요약할 수 있는 기능입니다.

#### 6.1 주요 기능
| 기능 | 설명 |
|------|------|
| 세션 요약 버튼 | 세션 초기화 팝업에서 요약 생성 버튼 표시 |
| AI 기반 요약 | 기본 모델을 사용하여 대화 내용 요약 |
| 요약 복사 | 생성된 요약 내용을 클립보드에 복사 |
| 다시 생성 | 요약이 만족스럽지 않으면 다시 생성 가능 |
...
#### 6.3 추가된 변수
```javascript
// ----- [2026-01-22] 세션 요약 기능 변수 시작 -----
let isGeneratingSummary = false; // 요약 생성 중 상태
let sessionSummary = ''; // 생성된 요약 내용
let showSummaryResult = false; // 요약 결과 표시 여부
// ----- [2026-01-22] 세션 요약 기능 변수 종료 -----
```
...
#### 6.5 UI 구성
- 파란색 섹션으로 요약 기능 영역 구분
- 로딩 스피너로 요약 생성 중 상태 표시
- 요약 결과는 스크롤 가능한 영역에 표시
- 클립보드 복사 버튼 제공

#### 6.6 사용 흐름
```
1. 토큰/턴 제한 초과 → 세션 초기화 팝업 표시
2. "세션 요약 생성 (~4000 토큰)" 버튼 클릭
3. AI가 대화 내용 분석 및 요약 생성 (로딩 표시)
4. 요약 결과 표시
5. "📋 복사" 버튼으로 클립보드에 복사
6. "새 채팅 시작" 버튼으로 새 세션 시작
```
